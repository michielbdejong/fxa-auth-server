...
:lock: OAuth Bearer token with scope `profile`, or HAWK-authenticated with sessionToken
...
The request must include an OAuth Bearer token with the scope of `profile`, or a HAWK header that authenticates the request using a `sessionToken` received from `/account/login`.
